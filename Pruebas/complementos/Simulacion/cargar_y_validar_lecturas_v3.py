...
def main():
    ...
    with Pool(NUM_PROCESSES, initializer=init_worker) as pool:
        for reads, errs in pool.imap_unordered(procesar_archivo, archivos):
            ...
            # Informamos también cuántos archivos ya se han procesado
            print(
    ...
    # 3) Bulk insert concurrente
    print("→ Inyectando lecturas en Cassandra...")
    execute_concurrent_with_args(session, read_ps, all_reads, concurrency=CONCURRENCY)

    print("→ Inyectando errores en Cassandra...")
    execute_concurrent_with_args(session, err_ps,  all_errs,  concurrency=CONCURRENCY)

    elapsed = time.time() - t0
    m, s = divmod(int(elapsed), 60)
    print(f"\n🎉 ¡Hecho en {m}m{s}s! Insertadas {len(all_reads)} lecturas y {len(all_errs)} errores.")
